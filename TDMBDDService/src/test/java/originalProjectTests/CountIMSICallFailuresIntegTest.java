...
public class CountIMSICallFailuresIntegTest {
	@Deployment
	...
	@Test
	public void testCountCallFailuresPerImsiSuccess() {
		List callFailuresCount= callDataDAO.countImsiFailuresForDuration(344930000000012L, startDate, endDate);
		...
		baseDataList.add(baseData);
		callDataDAO.addBaseData(baseDataList);
		List callFailures= callDataDAO.countImsiFailuresForDuration(344930000000012L, startDate, endDate);
		assertEquals(callFailures.size(), 1);
		
	}

	@Test
	public void checkCountAndDurationSuccess() {
		Response response = callDataWS.countIMSICallFailures(344930000000011L, startDate,endDate);
		assertEquals(HttpStatus.SC_OK, response.getStatus());
		
		List<Object[]> imsiCountDuration = (List<Object[]>) response.getEntity();
		
		assertEquals(imsiCountDuration.size(), 1);
	}
	/* differend user story - comment out for now
	@Test
	public void testCountCallFailuresPerImsiInvalidDate() {
		List callFailuresCount = callDataDAO.countImsiFailures(344930000000012L,startDate, endDate);
		assertEquals(callFailuresCount.size(), 0);
